...
public static class LicensePlateChanging
{
    ...
    public static void RemoveNearestLicensePlate()
    {
        ...
        {
            GTAVehicle VehicleToChange = InstantAction.TrackedVehicles.Where(x => x.VehicleEnt.Handle == ClosestVehicle.Handle).FirstOrDefault();
            if (VehicleToChange == null)
            {
                ...
                InstantAction.TrackedVehicles.Add(VehicleToChange);
            }
    ...
    public static void ChangeLicensePlateAnimation(GTAVehicle VehicleToChange, bool ChangePlates)
    {
        ...
        GameFiber ChangeLicensePlateAnimation = GameFiber.StartNew(delegate
        {
            try
            {
                ...
                if (!InstantAction.MovePedToCarPosition(VehicleToChange.VehicleEnt, Game.LocalPlayer.Character, VehicleToChange.VehicleEnt.Heading, ChangeSpot, true))
                    return;

                PlayerChangingPlate = true;
                Screwdriver = InstantAction.AttachScrewdriverToPed(Game.LocalPlayer.Character);
                if (ChangePlates)
                    LicensePlate = AttachLicensePlateToPed(Game.LocalPlayer.Character);

                InstantAction.RequestAnimationDictionay("mp_car_bomb");
