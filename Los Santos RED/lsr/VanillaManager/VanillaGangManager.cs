...
public class VanillaGangManager
{
   ...
   // private float LargeScenarioDistance => 50f;
    //private float ScenarioBlockingDistance => Settings.SettingsManager.VanillaSettings.BlockOtherGangScenariosDIstance; //goes in both directions so within 400 meters, no scenarios will be spawned
    ...
    private void SetGangScenarioBlocking(bool IsBlocked)
    {
        if (IsBlocked)
        {
            ...
            //{
            //    NativeFunction.Natives.ADD_SCENARIO_BLOCKING_AREA<int>(areas.X - LargeScenarioDistance, areas.Y - LargeScenarioDistance, areas.Z - LargeScenarioDistance, areas.X + LargeScenarioDistance, areas.Y + LargeScenarioDistance, areas.Z + LargeScenarioDistance, false, true, true, true);
            //}
