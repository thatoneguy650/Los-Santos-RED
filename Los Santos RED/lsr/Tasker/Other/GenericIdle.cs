...
public class GenericIdle : ComplexTask
{
    ...
    private bool shouldGuard = false;
            ...
            else
            {
                if (GameTimeLastStartedScenario > 0 && Game.GameTime - GameTimeLastStartedScenario >= GameTimeBetweenScenarios)
                {
                    if (RandomItems.RandomPercent(10f))//10 percent let tham transition to foot patrol people
                    {
                        IsGuarding = false;
                        IsPatrolling = true;
                        EntryPoint.WriteToConsole($"PED {Ped.Handle} TRANSITIONED FROM GUARDING TO PATROLLING");
                    }
    ...
    private void FootPatrol(bool IsFirstRun)
    {
        if (Ped.Pedestrian.Exists())
        {
            ...
            else
            {
                ...
                {
                    if(shouldGuard && RandomItems.RandomPercent(10f))
                    {
                        IsGuarding = true;
                        IsPatrolling = false;
                        EntryPoint.WriteToConsole($"PED {Ped.Handle} TRANSITIONED FROM PATROLLING TO GUARDING");
                    }
