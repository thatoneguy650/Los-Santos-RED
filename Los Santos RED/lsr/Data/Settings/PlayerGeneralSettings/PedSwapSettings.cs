...
{
    [Description("Uses memory editing to set the model name of whatever ped you are playing as to be the of one of the three main characters. This edit will let you do things only main characters are allowed to do (like spend money, shop, have random encounters, own property, etc.). Since this is overwriting memory, there is no longer a way for any other process to tell what model you are actually playing as, it will always return either player_zero (Michael) player_one (Franklin) or player_two (Trevor) based on the MainCharacterToAlias setting below to any other mods/trainers/missions etc. If you only play as one of the main three characters you can disable this option, however ped swapping will not function as expected. ")]
