...
public class PoliceSpeechManager
{
    ...
    public void Tick()
    {
        ...
        {
            Mod.Debug.WriteToLog(e.Message, e.StackTrace);
        }
    ...
    private void CheckSpeech()
    {
        foreach (SpeakingCop Cop in SpeakingCops.Where(x => x.AssignedCop.Pedestrian.Exists() && !x.AssignedCop.Pedestrian.IsDead))
        {
            ...
            if (Cop.CanRadioIn && Cop.AssignedCop.CanRecognizePlayer && Mod.Player.Violations.IsViolatingAnyLaws)
            {
                Cop.RadioIn();
            }
        }
    ...
    private class SpeakingCop
    {
        ...
        public void Speak()
        {
            ...
            {
                AssignedCop.Pedestrian.PlayAmbientSpeech(Mod.World.PoliceSpeechManager.RegularChaseSpeech.PickRandom());
            }
            else if (Mod.Player.IsNotWanted && Mod.Player.Respawning.RecentlyBribedPolice)
            {
                AssignedCop.Pedestrian.PlayAmbientSpeech(Mod.World.PoliceSpeechManager.AmbientSpeech.PickRandom());
            }
            else if (Mod.Player.CurrentPoliceResponse.IsDeadlyChase)
            {
                AssignedCop.Pedestrian.PlayAmbientSpeech(Mod.World.PoliceSpeechManager.DeadlyChaseSpeech.PickRandom());
            }
            else //Normal State
            {
        ...
        public void RadioIn()
        {
            ...
            WeaponInformation CurrentGun = Mod.DataMart.Weapons.GetCurrentWeapon(AssignedCop.Pedestrian);
