...
public class Tasking
{
    ...
    private class TaskableCop
    {
        ...
        private bool WithinChaseDistance
        {
            get
            {
                if (CopToTask.DistanceToPlayer <= Mod.World.PolicePerception.ActiveDistance)
                    return true;
                else
                    return false;
            }
        ...
        private bool WithinInvestigationDistance
        {
            get
            {
                if (CopToTask.DistanceToInvestigationPosition <= Mod.Player.Investigations.InvestigationDistance)
                    return true;
                else
                    return false;
            }
        ...
        private Activities CurrentActivity
        {
            get
            {
                ...
                {
                    if (Mod.Player.Investigations.InInvestigationMode && WithinInvestigationDistance)
                    {
        ...
        private void Chase()
        {
            if (WithinChaseDistance)
            {
                if (Mod.World.PolicePerception.AnyRecentlySeenPlayer && !Mod.Player.AreStarsGreyedOut)
                {
        ...
        {
            CopToTask.Pedestrian.BlockPermanentEvents = false;
            Vector3 WantedCenter = Mod.World.PolicePerception.PlaceLastSeenPlayer;//NativeFunction.CallByName<Vector3>("GET_PLAYER_WANTED_CENTRE_POSITION", Game.LocalPlayer);
            if (Mod.Player.IsInVehicle)
            {
        ...
        private Activities CurrentActivity
        {
            get
            {
                ...
                else
                {
                    ...
                    else if(CivilianToTask.CanRecognizePlayer && Mod.Player.Violations.IsViolatingAnyCivilianReportableCrime)
                    {
                        return Activities.ReactToCrime;
                    }
                    else if (CivilianToTask.WithinWeaponsAudioRange && Mod.Player.Violations.IsViolatingAnyAudioBasedCivilianReportableCrime)
                    {
                        return Activities.ReactToCrime;
                    }
                }
