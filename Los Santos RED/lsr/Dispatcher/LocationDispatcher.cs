    ...
    public void Dispatch()
    {
        foreach (InteractableLocation ps in World.Places.ActiveInteractableLocations.ToList().Where(x => x.IsEnabled && x.DistanceToPlayer <= x.ActivateDistance && x.IsNearby && !x.IsDispatchFilled && (x.PossiblePedSpawns != null || x.PossibleVehicleSpawns != null)).ToList())
        {
            ...
            if (ps.PossiblePedSpawns != null)
            {
                ...
                {
                    cl.AttemptSpawn(Player, true, false, Agencies, Gangs, Zones, Jurisdictions, GangTerritories, Settings, World, ps.AssociationID, Weapons, Names, Crimes, PedGroups,ShopMenus, WeatherReporter, Time);
            ...
            if (ps.PossibleVehicleSpawns != null)
            {
                foreach (ConditionalLocation cl in ps.PossibleVehicleSpawns)
                {
                    ...
                    cl.AttemptSpawn(Player, false, hasStuff, Agencies, Gangs, Zones, Jurisdictions, GangTerritories, Settings, World, ps.AssociationID, Weapons, Names, Crimes, PedGroups, ShopMenus, WeatherReporter, Time);
                    GameFiber.Yield();
                }
            }
