...
public class LocationsTab
{
    ...
    {
        List<TabItem> items = new List<TabItem>();
        List<BasicLocation> DirectoryLocations = PlacesOfInterest.GetAllLocations().Where(x => (x.ShowsOnDirectory || Settings.SettingsManager.WorldSettings.ShowAllLocationsOnDirectory) && x.IsEnabled && (FilterString == "" || x.Name.ToLower().Contains(FilterString.ToLower()))).ToList();
