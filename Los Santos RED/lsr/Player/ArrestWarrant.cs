...
public class ArrestWarrant
{
    ...
    public void Update()
    {
        if (!Mod.Player.IsDead && !Mod.Player.IsBusted)
        {
            ...
            {
                if (!PlayerIsPersonOfInterest && Mod.World.Police.AnyCanSeePlayer)
                {
    ...
    {
        if ((Mod.Player.IsNotWanted || Mod.Player.WantedLevel == 1) && Mod.World.Police.AnyCanRecognizePlayer && Mod.Player.IsInVehicle && Game.LocalPlayer.Character.IsInAnyVehicle(false))//first check is cheaper, but second is required to verify
        {
