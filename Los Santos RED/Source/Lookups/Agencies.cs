...
public static partial class Agencies
{
    ...
    public static List<Agency> GetAllSpawnableAgencies(Vector3 Position)
    {
        List<Agency> ToReturn = new List<Agency>();
        Street StreetAtPosition = Streets.GetStreet(Position);
        if(StreetAtPosition != null && Streets.GetStreet(Position).IsHighway) //Highway Patrol Jurisdiction
        {
            ToReturn.AddRange(AgenciesList.Where(x => x.CanSpawn && x.SpawnsOnHighway));
        }
        Zone CurrentZone = Zones.GetZone(Position);

        Agency ZoneAgency1 = Jurisdiction.GetRandomAgency(CurrentZone.InternalGameName);
        if (ZoneAgency1 != null)
        {
            ToReturn.Add(ZoneAgency1); //Zone Jurisdiciton Random
        }

        Agency CountyAgency1 = Jurisdiction.GetRandomCountyAgency(CurrentZone.InternalGameName);
    ...
    public static Agency GetAgency(Ped Cop)
    {
        if (!Cop.IsPoliceArmy())
        {
            return null;
        ...
        else if (Cop.IsPolice())
        {
            ...
            if (ModelMatchAgencies.Count > 1)
            {
                ...
                if (ZoneFound != null)
                {
                    foreach (Agency ZoneAgency in Jurisdiction.GetAllAgencies(ZoneFound.InternalGameName))
                    {
    ...
    public static Agency GetAgency(Vehicle CopCar)
    {
        Agency ToReturn;
