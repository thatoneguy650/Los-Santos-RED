...
public class Agency
{
    ...
    public PedestrianInformation GetRandomPed(List<string> RequiredModels)
    {
        ...
        List<PedestrianInformation> ToPickFrom = CopModels.Where(x => PlayerState.WantedLevel >= x.MinWantedLevelSpawn && PlayerState.WantedLevel <= x.MaxWantedLevelSpawn).ToList();
