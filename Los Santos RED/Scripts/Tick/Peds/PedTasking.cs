...
public static class PedTasking
{   
    ...
    private class TaskableCop
    {
        ...
        public string CurrentTask
        {
            get
            {
                if (PlayerState.IsWanted)
                {
                    if (TaskGTACop.DistanceToPlayer <= Police.ActiveDistance)
                    {
                        return CurrentTaskActive;
                    }
                    else
                    {
                        return "Idle";
                ...
                {
                    float DistToInvest = TaskGTACop.Pedestrian.DistanceTo2D(Investigation.InvestigationPosition);
        ...
        private string CurrentTaskActive
        {
            get
            {
                if (TaskGTACop.IsInVehicle)
                {
                    if (TaskGTACop.IsDriver)
                    {
                        if (TaskGTACop.DistanceToPlayer <= 25f && 1 == 0)//Can Vehicle Chase?
                        {
                            return "ChasePlayerOnFootWithVehicle";
                        }
                        else if (TaskGTACop.DistanceToPlayer >= 100f)
                        {
                            return "DriveToPlayer";
                        }
                        else
                        {
                            return "NoTask";
                ...
                else
                {
                    if (PlayerState.IsInVehicle)
                    {
                        if (PlayerState.IsStationary && TaskGTACop.DistanceToPlayer <= 50f && Police.AnyRecentlySeenPlayer)
                        {
                            return "Carjack Player";
                        }
                        else
                        {
                            return "NoTask";
                    ...
                    {
                        if (TaskGTACop.DistanceToPlayer <= 50f && Police.AnyRecentlySeenPlayer)
                        {
                            return "ChasePlayerOnFoot";
                        }
                        else
                        {
                            return "NoTask";
        ...
        {
            if (!TaskGTACop.Pedestrian.Exists() || (TaskGTACop.SetUnarmed && !TaskGTACop.NeedsWeaponCheck))
                return;
            if (General.MySettings.Police.OverridePoliceAccuracy)
                TaskGTACop.Pedestrian.Accuracy = General.MySettings.Police.PoliceGeneralAccuracy;

            NativeFunction.CallByName<bool>("SET_PED_SHOOT_RATE", TaskGTACop.Pedestrian, 0);
            if (!(TaskGTACop.Pedestrian.Inventory.EquippedWeapon == null))
            {
                NativeFunction.CallByName<bool>("SET_CURRENT_PED_WEAPON", TaskGTACop.Pedestrian, (uint)2725352035, true); //Unequip weapon so you don't get shot
                NativeFunction.CallByName<bool>("SET_PED_CAN_SWITCH_WEAPON", TaskGTACop.Pedestrian, false);
            }
            NativeFunction.CallByName<bool>("SET_PED_COMBAT_ATTRIBUTES", TaskGTACop.Pedestrian, 2, false);//cant do drivebys
            TaskGTACop.SetTazer = false;
            TaskGTACop.SetUnarmed = true;
            TaskGTACop.SetDeadly = false;
            TaskGTACop.GameTimeLastWeaponCheck = Game.GameTime;
        }
        private void SetCopDeadly()
        {
            if (!TaskGTACop.Pedestrian.Exists() || (TaskGTACop.SetDeadly && !TaskGTACop.NeedsWeaponCheck))
                return;
            if (General.MySettings.Police.OverridePoliceAccuracy)
                TaskGTACop.Pedestrian.Accuracy = General.MySettings.Police.PoliceGeneralAccuracy;
            NativeFunction.CallByName<bool>("SET_PED_SHOOT_RATE", TaskGTACop.Pedestrian, 30);
            if (!TaskGTACop.Pedestrian.Inventory.Weapons.Contains(TaskGTACop.IssuedPistol.Name))
                TaskGTACop.Pedestrian.Inventory.GiveNewWeapon(TaskGTACop.IssuedPistol.Name, -1, true);

            if ((TaskGTACop.Pedestrian.Inventory.EquippedWeapon == null || TaskGTACop.Pedestrian.Inventory.EquippedWeapon.Hash == WeaponHash.StunGun) && Game.LocalPlayer.WantedLevel >= 0)
                TaskGTACop.Pedestrian.Inventory.GiveNewWeapon(TaskGTACop.IssuedPistol.Name, -1, true);

            if (TaskGTACop.IssuedHeavyWeapon != null)
            {
                NativeFunction.CallByName<bool>("SET_CURRENT_PED_WEAPON", TaskGTACop.Pedestrian, NativeFunction.CallByName<bool>("GET_BEST_PED_WEAPON", TaskGTACop.Pedestrian, 0), true);
            }

            if (General.MySettings.Police.AllowPoliceWeaponVariations)
                General.ApplyWeaponVariation(TaskGTACop.Pedestrian, (uint)TaskGTACop.IssuedPistol.Hash, TaskGTACop.PistolVariation);
            NativeFunction.CallByName<bool>("SET_PED_CAN_SWITCH_WEAPON", TaskGTACop.Pedestrian, true);
            NativeFunction.CallByName<bool>("SET_PED_COMBAT_ATTRIBUTES", TaskGTACop.Pedestrian, 2, true);//can do drivebys

            TaskGTACop.SetTazer = false;
            TaskGTACop.SetUnarmed = false;
            TaskGTACop.SetDeadly = true;
            TaskGTACop.GameTimeLastWeaponCheck = Game.GameTime;
        }
        private void SetCopTazer()
        {
            if (!TaskGTACop.Pedestrian.Exists() || (TaskGTACop.SetTazer && !TaskGTACop.NeedsWeaponCheck))
                return;

            if (General.MySettings.Police.OverridePoliceAccuracy)
                TaskGTACop.Pedestrian.Accuracy = General.MySettings.Police.PoliceTazerAccuracy;
            NativeFunction.CallByName<bool>("SET_PED_SHOOT_RATE", TaskGTACop.Pedestrian, 100);
            if (!TaskGTACop.Pedestrian.Inventory.Weapons.Contains(WeaponHash.StunGun))
            {
                TaskGTACop.Pedestrian.Inventory.GiveNewWeapon(WeaponHash.StunGun, 100, true);
            }
            else if (TaskGTACop.Pedestrian.Inventory.EquippedWeapon != WeaponHash.StunGun)
            {
                TaskGTACop.Pedestrian.Inventory.EquippedWeapon = WeaponHash.StunGun;
            }
            NativeFunction.CallByName<bool>("SET_PED_CAN_SWITCH_WEAPON", TaskGTACop.Pedestrian, false);
            NativeFunction.CallByName<bool>("SET_PED_COMBAT_ATTRIBUTES", TaskGTACop.Pedestrian, 2, false);//cant do drivebys
            TaskGTACop.SetTazer = true;
            TaskGTACop.SetUnarmed = false;
            TaskGTACop.SetDeadly = false;
            TaskGTACop.GameTimeLastWeaponCheck = Game.GameTime;
        }
