    ...
    public static void ChangeLicensePlateAnimation(GTAVehicle VehicleToChange, bool ChangePlates)
    {
        ...
        GameFiber ChangeLicensePlateAnimation = GameFiber.StartNew(delegate
        {
            try
            {
                ...
                if (!LosSantosRED.MovePedToCarPosition(VehicleToChange.VehicleEnt, Game.LocalPlayer.Character, VehicleToChange.VehicleEnt.Heading, ChangeSpot, true))
                    return;

                PlayerChangingPlate = true;
                Screwdriver = LosSantosRED.AttachScrewdriverToPed(Game.LocalPlayer.Character);
                if (ChangePlates)
                    LicensePlate = AttachLicensePlateToPed(Game.LocalPlayer.Character);

                LosSantosRED.RequestAnimationDictionay("mp_car_bomb");
