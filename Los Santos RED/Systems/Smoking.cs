...
public static class Smoking
{
    ...
    private static void MainLoop()
    {
        GameFiber.StartNew(delegate
        {
            try
            {
                while (IsRunning)
                {
                    if (LosSantosRED.IsDead || Game.LocalPlayer.Character.IsRagdoll)
                    {
                        RemoveCigarette();
                        return;
                    }
    ...
    {
        SetPedUnarmed(Game.LocalPlayer.Character, false);
        LosSantosRED.RequestAnimationDictionay(CurrentSmokingAnimation.EnterAnimationDictionary);
        LosSantosRED.RequestAnimationDictionay(CurrentSmokingAnimation.BaseAnimationDictionary);
    ...
    private static void AttachCigaretteToPedLowerLip(Ped Pedestrian)
    {
        if (!PlayersCurrentCigarette.Exists())
        {
            ...
            LosSantosRED.CreatedObjects.Add(PlayersCurrentCigarette);
        }
    ...
    private static void StopPuffingCigarette(bool PlayAnimation)
    {
        if (PlayAnimation)
        {
            LosSantosRED.RequestAnimationDictionay(CurrentSmokingAnimation.ExitAnimationDictionary);
