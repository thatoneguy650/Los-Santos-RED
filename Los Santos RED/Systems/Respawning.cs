...
public static class Respawning
{
    ...
    public static void BribePolice(int Amount)
    {
        ...
        else
        {
            ...
            GTACop ClosestCop = PedScanning.CopPeds.Where(x => x.Pedestrian.Exists() && x.Pedestrian.IsAlive).OrderBy(x => x.DistanceToPlayer).FirstOrDefault();
            if (ClosestCop == null)
            {
    ...
    public static void RespawnInPlace(bool AsOldCharacter)
    {
        try
        {
            ...
            if (LosSantosRED.DiedInVehicle)
            {
                ...
                LosSantosRED.DiedInVehicle = false;
            }
            else
            {
    ...
    public static void RemoveIllegalWeapons()
    {
        ...
        foreach (DroppedWeapon MyNewGun in MyOldGuns)
        {
            ...
            if (MyGTANewGun == null || MyGTANewGun.IsLegal)//or its an addon gun
            {
                ...
                LosSantosRED.ApplyWeaponVariation(Game.LocalPlayer.Character, (uint)MyNewGun.Weapon.Hash, MyNewGun.Variation);
