...
public static class Debugging
{
    ...
    private static void DebugNumpad4()
    {
        ...
        //        NativeFunction.CallByName<bool>("REQUEST_STREAMED_TEXTURE_DICT", "CHAR_CALL911", true);
        //        NativeFunction.CallByName<bool>("REQUEST_STREAMED_TEXTURE_DICT", "char_call911", true);


        //        while (!NativeFunction.CallByName<bool>("HAS_STREAMED_TEXTURE_DICT_LOADED", "char_call911"))
        //            GameFiber.Yield();
        ...
        //handle = NativeFunction.Natives.RegisterPedheadshot<int>(Game.LocalPlayer.Character);
        ...
        //    unsafe
        //    {
        //        IntPtr ptr = NativeFunction.CallByName<IntPtr>("GET_DISPLAY_NAME_FROM_VEHICLE_MODEL", Game.LocalPlayer.Character.CurrentVehicle.Model.Hash);
        //        ModelName = Marshal.PtrToStringAnsi(ptr);
        //    }
        //    unsafe
        //    {
        //        IntPtr ptr2 = NativeFunction.CallByHash<IntPtr>(0x7B5280EBA9840C72, ModelName);
        //        ModelName2 = Marshal.PtrToStringAnsi(ptr2);
        //    }
        //    WriteToLog("PlayerCurrentCarNiceName", string.Format("NiceName: {0},{1}", ModelName, ModelName2));


        //    DispatchAudio.ReportStolenVehicle(LosSantosRED.GetPlayersCurrentTrackedVehicle());

        //    System.Drawing.Color MyColor = LosSantosRED.GetPlayersCurrentTrackedVehicle().VehicleEnt.PrimaryColor;
        //    string FoundName = ColorMapper.GetNearestName(MyColor);
        //    WriteToLog("PlayerCurrentCarColorName", string.Format("Color: {0}, FoundName: {1}", MyColor.Name,FoundName));
        //}
    ...
    private static void DebugNumpad5()
    {
        ...
        //DispatchAudio.ReportLowLevelCiviliansKilled();

        if (!Game.LocalPlayer.Character.IsInAnyVehicle(false))
            return;
        //DispatchAudio.ReportFelonySpeeding(InstantAction.GetPlayersCurrentTrackedVehicle(), 110f);
        GTAVehicle VehicleDescription = LosSantosRED.TrackedVehicles.Where(x => x.VehicleEnt.Handle == Game.LocalPlayer.Character.CurrentVehicle.Handle).FirstOrDefault();
       ...
       // Debugging.WriteToLog("Civilians", string.Format("Total Civilians: {0}", PoliceScanning.Civilians.Count()));
        Debugging.WriteToLog("Civilians", string.Format("PlayerInAutomobile: {0}", LosSantosRED.PlayerInAutomobile));
        ...
        foreach (Vehicle MyCar in TrafficViolations.CloseVehicles.Where(x => x.Exists()))
        {
            ...
            {
                Debugging.WriteToLog("RunningRed", string.Format("IS_VEHICLE_STOPPED_AT_TRAFFIC_LIGHTS", LosSantosRED.PlayerInVehicle));
    ...
    private static void DebugNumpad6()
    {
        try
        {
            ...
            //if(FoundPlace)
            //    Game.LocalPlayer.Character.Position = PlacedPosition;
    ...
    private static void DebugNumpad8()
    {
        ...
        Debugging.WriteToLog("Debug", "-------Criminal History---------");
        ...
        {
            Debugging.WriteToLog("Player Killed", string.Format("Handle: {0}, Distance: {1}",DeadPerson.Pedestrian.Handle, Game.LocalPlayer.Character.DistanceTo2D(DeadPerson.Pedestrian)));


        }

        Debugging.WriteToLog("Debug", string.Format("NearMurderVictim: {0}", Civilians.NearMurderVictim(15f)));
